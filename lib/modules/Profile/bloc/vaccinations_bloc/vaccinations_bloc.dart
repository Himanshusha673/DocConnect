...
import 'package:doc_connect/modules/Profile/services/vaccine_repository.dart';
...
class VaccinationsBloc extends Bloc<VaccinationsEvent, VaccinationsState> {
  VaccinationsBloc() : super(VaccinationsState()) {
    ...
    on<VaccinationsGetVaccineListEvent>((event, emit) async {
      ...
      if (res != null) {
        List<Vaccine> vacines = res.data
            ...
            .toList();
        emit(state.copyWith(
            vaccineList: vacines, vaccineName: null, date: null, image: null));
      }
    ...
    on<VaccinationsAddVaccinationDetailEvent>((event, emit) async {
      if (state.image != null) {
        String? imageUrl =
            await VaccineRepository().uploadReportFile(file: state.image!);
        if (imageUrl != null) {
          DateTime date = DateTime.parse(state.date!);
          DateTime vacDate =
              DateTime.utc(date.year, date.month, date.day, 20, 18, 04);

          Response<PostVaccineResponse>? res =

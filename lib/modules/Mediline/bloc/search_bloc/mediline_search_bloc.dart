...
class MedilineSearchBloc extends Bloc<MedilineSearchEvent, MedilineSearchState> {
  MedilineSearchBloc() : super(MedilineSearchState()) {
    on<MedilineGetSearchEvent>((event, emit) async {
      emit(MedilineSearchloadingState());
      GetAppointmentResponse? res = await MedilineRepository().getAppointmentsSearch(search: event.searchQuery);
